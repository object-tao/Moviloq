...
import { roles, can, type ResourceRow, type DocumentRow, type Role } from "../shared/operations";
...
operations.get("/",async(c,next)=>{
  ...
  const todo=panel(t(v,["审核与补件待办", "Review & information requests"]),table(v,[["档案", "Record"],["类型", "Type"],["状态", "Status"]],rows.results.map(row=>[link(`/ops/resource/${row.id}`,row.name),h(label(v,row.kind)),badge(v,row.status)])),link("/ops/reviews",t(v,["全部审核 →", "All reviews →"])));
...
operations.get("/ops/reviews",async c=>{
  requirePermission(c,"resources:read");const v=view(c);const num=pageNumber(c);const status=c.req.query("status")||"submitted";if(!["submitted","needs_info","approved","rejected","expiring"].includes(status))throw new OpsError("INVALID_INPUT");
  const rows=await db(c).prepare(`SELECT d.id,d.resource_id,d.document_type,d.status,d.filename,d.expires_on,r.name FROM ops_documents d JOIN ops_resources r ON r.id=d.resource_id WHERE ${latestDocument} AND ((?='expiring' AND d.status='approved' AND d.expires_on<=?) OR d.status=?) ORDER BY d.updated_at,d.id LIMIT 26 OFFSET ?`).bind(status,new Date(Date.now()+30*86400000).toISOString().slice(0,10),status,(num-1)*25).all<DocumentRow&{name:string}>();
  const nav=`<div class="toolbar">${["submitted","needs_info","approved","rejected","expiring"].map(key=>link(`/ops/reviews?status=${key}`,key==="expiring"?t(v,["临近到期 / 已过期", "Expiring / overdue"]):label(v,key),key===status?"badge submitted":"")).join("")}</div>`;
  const records=await db(c).prepare("SELECT * FROM ops_resources WHERE status=? ORDER BY updated_at,id LIMIT 26 OFFSET ?").bind(status==="expiring"?"submitted":status,(num-1)*25).all<ResourceRow>();
  return c.html(page(v,"reviews",t(v,["资料审核中心", "Document review centre"]),panel(t(v,["最新提交的资料", "Latest document submissions"]),nav+table(v,[["档案", "Record"],["资料类型", "Document type"],["状态", "Status"],["有效至", "Valid through"],["文件", "File"]],rows.results.slice(0,25).map(row=>[link(`/ops/resource/${row.resource_id}`,row.name),h(label(v,row.document_type)),badge(v,row.status),h(row.expires_on),link(`/ops/document/${row.id}`,row.filename)]))+pagination(v,`/ops/reviews?status=${status}`,num,rows.results.length>25))+panel(t(v,["档案审核队列", "Record review queue"]),table(v,[["档案", "Record"],["类型", "Type"],["状态", "Status"]],records.results.slice(0,25).map(row=>[link(`/ops/resource/${row.id}`,row.name),h(label(v,row.kind)),badge(v,row.status)]))+pagination(v,`/ops/reviews?status=${status}`,num,records.results.length>25)),t(v,["审核文件后还需审核档案。重交的最新版本优先；旧版保留在档案详情中。", "Approve documents, then review the parent record. The latest submission takes precedence; earlier versions remain in record details."])));
});
...
operations.route("/",resourceRoutes);operations.route("/",documentRoutes);operations.route("/",configRoutes);operations.route("/",staffRoutes);
