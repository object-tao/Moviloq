...
operations.onError((error,c)=>{
  ...
  if((/^\/ops\/resources\/(fleet|driver)\/create$/.test(c.req.path)||/^\/ops\/reviews\/resource\/[^/]+\/status$/.test(c.req.path))&&c.req.header("Accept")==="application/json")return c.json({error:code,message:message+details},status);
