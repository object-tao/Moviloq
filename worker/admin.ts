...
import { driverReviewHash, driverReviewCss } from "./ops-driver-review-dialog";
...
export function createAdminApp() {
  ...
  app.use("*", (c,next)=>secureHeaders({ referrerPolicy: "same-origin", contentSecurityPolicy: {
    ...
    ...(c.req.method==="GET"&&["/ops/resources/fleet","/ops/resources/driver"].includes(c.req.path) ? { scriptSrc: [`'${resourceDialogHash}'`,...(c.req.path==="/ops/resources/driver"?[`'${driverReviewHash}'`]:[])], connectSrc: ["'self'"] } : {})
  } })(c,next));
  ...
  app.get("/ops.css", c => c.body(opsCss + opsNavigationCss + resourceDialogCss + driverReviewCss, 200, { "Content-Type": "text/css; charset=utf-8" }));
