...
resourceRoutes.get("/ops/resources/:kind", async c => {
  ...
  return c.html(page(v,kind,label(v,kind),panel(t(v,["档案列表", "Records"]),filters+table(v,[["名称", "Name"],["状态", "Status"],["资料检查", "Readiness"],["版本", "Version"],["更新时间（UTC）", "Updated (UTC)"]],visible.map(row=>[link(`/ops/resource/${row.id}`,row.name),badge(v,row.status),h(t(v,readiness(row,context.documents,context.configs,context.peers).length ? ["受限 / 待处理", "Blocked / needs attention"] : ["资料就绪", "Records ready"])),String(row.version),h(row.updated_at.slice(0,16).replace("T"," "))]))+pagination(v,`/ops/resources/${kind}?q=${encodeURIComponent(query)}&status=${encodeURIComponent(status)}`,num,rows.results.length>25)),t(v,["管理真实合作资料。新建档案不会创建司机登录账号。", "Manage actual partner records. Creating a record does not create a driver account."]),can(v.role,"resources:write") ? link(`/ops/resources/${kind}/new`,t(v,["＋ 新增档案", "+ Add record"]),"button-link") : ""));
});
...
resourceRoutes.get("/ops/resource/:id",async c=>{
  ...
  if(can(v.role,"resources:write") && ["draft","needs_info","rejected"].includes(row.status)) reviewButtons.push(submit(v,["提交审核", "Submit for review"],"action","submit"));
  if(can(v.role,"review:write")) {
    if(row.status==="submitted") reviewButtons.push(submit(v,["审核通过", "Approve"],"action","approve"),submit(v,["退回补件", "Request information"],"action","needs_info"),submit(v,["拒绝", "Reject"],"action","reject"));
    if(row.status==="approved") reviewButtons.push(submit(v,["暂停", "Suspend"],"action","suspend"));
    if(row.status==="suspended") reviewButtons.push(submit(v,["恢复至待审核", "Restore to review"],"action","restore"));
  }
  const review=reviewButtons.length ? panel(t(v,["审核与状态操作", "Review and status"]),form(v,`/ops/resource/${row.id}/status`,hidden("version",row.version)+reasonField(v)+reviewButtons.join(""))) : "";
...
resourceRoutes.post("/ops/resource/:id/status",async c=>{
  const action=value(c,"action");requirePermission(c,action==="submit"?"resources:write":"review:write");const row=await getResource(db(c),c.req.param("id"));checkVersion(row,value(c,"version"));const why=reason(c);
