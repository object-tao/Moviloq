...
if (!process.env.CLOUDFLARE_API_TOKEN || !/^[a-f0-9]{64}$/.test(ownerHash ?? "")) throw new Error("Provide the Cloudflare API token and confirmed owner SHA-256 through environment variables only.");
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (reset) {
      ...
      const result = await api("/query", { sql: "UPDATE admin_users SET password_hash = ?, credential_version = credential_version + 1, updated_at = ? WHERE id = 'owner' AND email_sha256 = ? AND status = 'active'", params: [encoded, at, ownerHash] });
      if (result[0].meta.changes !== 1) throw new Error("Owner changed during reset");
    } else {
      const result = await api("/query", { sql: "INSERT INTO admin_users (id,username,email_sha256,password_hash,created_at,updated_at) SELECT 'owner','admin',?,?,?,? WHERE NOT EXISTS (SELECT 1 FROM admin_users)", params: [ownerHash, encoded, at, at] });
    ...
    res.end("<h1>管理员密码已保存</h1><p>账号：admin。现在可以关闭此页面。若网站仍显示旧登录，请等待本次发布切换完成。</p><p>Password saved. You can close this local page.</p>");
