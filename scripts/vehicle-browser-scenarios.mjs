...
export async function verifyVehicleDialogs({page,db,origin,fleetId,destination}) {
  ...
  const vehiclePolicyLink=review.getByRole('alert').getByRole('link',{name:'Configure / publish checklist',exact:true});assert.equal(new URL(await vehiclePolicyLink.getAttribute('href')).searchParams.get('scope'),'vehicle:transporter');
