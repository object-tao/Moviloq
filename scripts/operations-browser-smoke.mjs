...
try {
  ...
  const guidance=reviewModal.getByRole('alert').locator('[data-review-issues]');assert(await guidance.isVisible());const checklistLink=guidance.getByRole('link',{name:'Configure / publish checklist',exact:true});assert.equal(new URL(await checklistLink.getAttribute('href')).searchParams.get('scope'),'driver');
  const openedPolicy=page.waitForEvent('popup');await checklistLink.click();const policyPage=await openedPolicy;await policyPage.waitForLoadState();assert.equal(new URL(policyPage.url()).searchParams.get('scope'),'driver');assert.equal(await policyPage.getByRole('link',{name:'+ New review checklist',exact:true}).getAttribute('href'),'/ops/configs/new?kind=requirements&scope=driver');await policyPage.close();assert.equal(await reviewModal.locator('[name="reason"]').inputValue(),'Preserve reason when documents are missing');
  for(const [size,width,height]of [['desktop',1440,1000],['mobile',390,844],['narrow',320,720]]){await page.setViewportSize({width,height});await reviewModal.getByRole('alert').focus();assert(await reviewModal.evaluate(element=>element.scrollWidth<=element.clientWidth));await page.screenshot({path:join(destination,`review-guidance-dialog-${size}.png`)});}await page.setViewportSize({width:1440,height:1000});
  ...
  const vehicleId=await verifyVehicleDialogs({page,db,origin,fleetId,destination});
  ...
  for(const [name,width,height,language]of [["desktop",1440,1000,"en"],["mobile",390,844,"zh"],["narrow",320,720,"en"]]){
    ...
    for(const [section,path]of [["overview","/"],["fleets","/ops/resources/fleet"],["drivers","/ops/resources/driver"],["vehicles","/ops/resources/vehicle"],["review","/ops/reviews"],["review-fleet","/ops/reviews/fleet?status=approved"],["review-driver","/ops/reviews/driver"],["review-vehicle","/ops/reviews/vehicle"],["rules","/ops/configs"],["staff","/ops/staff"],["audit","/ops/audit"]]){
      ...
      if(section==='drivers'||section==='vehicles'){
        const kind=section==='drivers'?'driver':'vehicle';const modal=page.locator(`#${kind}-review-dialog`);
        await page.locator(`[data-${kind}-create]`).click();const createModal=page.locator(`#${kind}-create-dialog`);assert(await createModal.isVisible());assert(await createModal.evaluate(element=>element.scrollWidth<=element.clientWidth));await page.screenshot({path:join(destination,`${kind}-create-dialog-${name}.png`)});await createModal.locator('[data-dialog-close]').first().click();
        await page.locator(`[data-${kind}-review]`).click();await modal.locator(`[data-${kind}-review-fragment]`).waitFor();assert.equal(new URL(page.url()).pathname,`/ops/resources/${kind}`);assert(await modal.evaluate(element=>element.scrollWidth<=element.clientWidth));assert(await modal.evaluate(element=>{const rect=element.getBoundingClientRect();return rect.top>=0&&rect.bottom<=window.innerHeight;}));assert(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth));await page.screenshot({path:join(destination,`${kind}-review-dialog-${name}.png`)});await modal.locator('[data-review-close]').click();assert(!(await modal.isVisible()));
      }
    }
  }
  ...
  for(const kind of ['fleet','driver','vehicle']){await fallbackPage.goto(origin+`/ops/resources/${kind}`);await fallbackPage.locator(`[data-${kind}-create]`).click();await fallbackPage.waitForURL(`**/ops/resources/${kind}/new`);assert(await fallbackPage.locator(`form[action="/ops/resources/${kind}/create"]`).isVisible());}
  for(const [kind,id]of [['driver',driverId],['vehicle',vehicleId]]){await fallbackPage.goto(origin+`/ops/resources/${kind}`);await fallbackPage.locator(`[data-${kind}-review]`).click();await fallbackPage.waitForURL(origin+`/ops/reviews/resource/${id}`);assert(await fallbackPage.locator('form[action$="/status"]').isVisible());}await noScript.close();
  ...
  assert.equal(await reviewerPage.locator('#driver-review-dialog [data-review-issues] a[href^="/ops/config"]').count(),0);
  await reviewerPage.goto(origin+'/ops/resources/vehicle');assert.equal(await reviewerPage.locator('[data-vehicle-create]').count(),0);await reviewerPage.locator('[data-vehicle-review]').click();await reviewerPage.locator('#vehicle-review-dialog form').waitFor();assert.equal(await reviewerPage.locator('#vehicle-review-dialog [value="suspend"]').count(),1);assert.equal(new URL(reviewerPage.url()).pathname,'/ops/resources/vehicle');
  ...
  // The original full-page error also shows actionable missing-document details.
  await page.goto(origin+'/ops/language/en');await page.goto(origin+'/ops/resources/fleet/new');
  for(const [key,value]of Object.entries({name:'Synthetic guidance fleet',legalName:'Synthetic guidance company',source:'Authorized isolated guidance test',reason:'Verify full-page review feedback'}))await page.locator(`form[action$="/create"] [name="${key}"]`).fill(value);
  await page.locator('form[action$="/create"] [name="authorized"]').check();await page.locator('form[action$="/create"]').getByRole('button',{name:'Save',exact:true}).click();await page.waitForURL(/saved=1$/);const guidanceId=new URL(page.url()).pathname.split('/')[3];await page.locator('form[action$="/status"] [name="reason"]').fill('Submit guidance fixture');await page.getByRole('button',{name:'Submit for review',exact:true}).click();await page.waitForURL(/saved=1$/);
  const guidanceRevision=(await db.prepare('SELECT revision FROM ops_meta WHERE id=1').first()).revision;
  for(const [size,width,height,lang]of [['desktop',1440,1000,'en'],['mobile',390,844,'zh'],['narrow',320,720,'en']]){
    await page.setViewportSize({width,height});await page.goto(origin+`/ops/language/${lang}`);await page.goto(origin+`/ops/reviews/resource/${guidanceId}`);await page.locator('form[action$="/status"] [name="reason"]').fill('Check missing required file');const blocked=page.waitForResponse(response=>response.url()===origin+`/ops/reviews/resource/${guidanceId}/status`&&response.status()===422);await page.locator('[value="approve"]').click();await blocked;await page.waitForURL(`**/ops/reviews/resource/${guidanceId}/status`);
    const issue=page.getByRole('alert').locator('[data-issue-code="document_missing"]');await issue.waitFor();assert.equal(await issue.locator('a').getAttribute('href'),`/ops/resource/${guidanceId}#documents`);assert(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth));await page.getByRole('alert').scrollIntoViewIfNeeded();await page.screenshot({path:join(destination,`review-guidance-page-${size}.png`)});await issue.locator('a').click();await page.waitForURL(`**/ops/resource/${guidanceId}#documents`);assert(await page.locator('#documents form[enctype="multipart/form-data"]').isVisible());
  }
  assert.equal((await db.prepare('SELECT revision FROM ops_meta WHERE id=1').first()).revision,guidanceRevision);
  assert.deepEqual(errors,[]);assert((await db.prepare("SELECT count(*) n FROM ops_events").first()).n>=8);
  await context.close();console.log("Verified isolated workerd/D1: actionable review guidance in full pages/dialogs, scoped checklist and document links, fleet/driver/vehicle creation dialogs, vehicle duplicate/dimension checks, review validation/concurrency/save/retry protection, private documents, fleet/vehicle approval, price simulation/publication, staff creation/forced change/RBAC/revocation, bilingual 1440/390/320px layouts, CSRF and anonymous denial. No production data or credentials used.");
